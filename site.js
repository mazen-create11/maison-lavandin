...
/* Codes de réduction : saisis sans tenir compte de la casse, valables jusqu’au jour « fin » inclus. */
const CODES = { "LAVANDIN10": { taux: 0.10, source: "tiktok", fin: "2026-12-31" } };
...
const CLE_CODE = "maison-lavandin-code";
...
/** @returns {{ nom: string, taux: number, source: string } | null} Un code inconnu ou expiré ne rend rien. */
function trouverCode(saisie) {
  const nom = String(saisie || "").trim().toUpperCase();
  const c = Object.prototype.hasOwnProperty.call(CODES, nom) ? CODES[nom] : null;
  if (!c) return null;
  const [annee, mois, jour] = c.fin.split("-").map(Number);
  if (Date.now() > new Date(annee, mois - 1, jour, 23, 59, 59, 999).getTime()) return null;
  return { nom, taux: c.taux, source: c.source };
}
function lireCode() {
  try { return trouverCode(localStorage.getItem(CLE_CODE)); } catch { return null; }
}
function ecrireCode(c) { try { if (c) localStorage.setItem(CLE_CODE, c.nom); else localStorage.removeItem(CLE_CODE); } catch { /* stockage indisponible : le code vit le temps de la page */ } }
...
/* Choix : les paliers cadeau (39,99 €) et livraison (49 €) se jugent sur totalPanier(), avant le code, qui ne fait jamais perdre un avantage. */
const remiseCode = () => (code ? Math.round(Math.round(totalPanier() * 100) * code.taux) / 100 : 0);
const totalAPayer = () => Math.round((totalPanier() - remiseCode()) * 100) / 100;
const pourcentCode = (c) => `−${Math.round(c.taux * 100)} %`;
/* ?code=LAVANDIN10 sur n’importe quelle page : le code est appliqué, mémorisé, puis retiré de l’adresse. */
const codeLien = (params.get("code") || "").slice(0, 30);
let codeDepuisLien = null;
let codeLienInconnu = "";
if (codeLien) {
  const trouve = trouverCode(codeLien);
  if (trouve) { code = trouve; codeDepuisLien = trouve; ecrireCode(trouve); signal("code", { code: trouve.nom, source: trouve.source, origine: "lien" }); }
  else if (!code) { codeLienInconnu = codeLien.trim().toUpperCase(); signal("code", { code: "inconnu", source: "", origine: "lien" }); }
  try { const url = new URL(window.location.href); url.searchParams.delete("code"); window.history.replaceState(window.history.state, "", url); } catch { /* adresse laissée telle quelle */ }
}
...
tiroir.innerHTML = `
  ...
  <div class="panier-pied">
    ...
    <form class="panier-code" hidden><label for="panier-code-champ">Code</label><input id="panier-code-champ" name="code" type="text" maxlength="30" autocomplete="off" autocapitalize="characters" spellcheck="false" enterkeyhint="done" aria-describedby="panier-code-message"><button class="panier-code-appliquer" type="button">Appliquer</button></form>
    <p class="panier-code-message" id="panier-code-message" role="status" aria-live="polite"></p>
    <div class="panier-remises" hidden></div>
    <div class="panier-recap panier-recap-sous-total"><span>Sous-total</span><strong class="panier-sous-total"></strong></div>
    <div class="panier-code-recap" hidden>
      <div class="panier-remise panier-remise-code"><span><span class="panier-code-libelle"></span> <button class="panier-code-retirer" type="button" aria-label="Retirer le code">Retirer</button></span><strong class="panier-code-montant"></strong></div>
      <div class="panier-recap panier-total"><span>Total</span><strong class="panier-total-montant"></strong></div>
    </div>
    <a class="bouton panier-commander" href="contact.html?panier=1#commande">${ICONE_PANIER}<span>Passer commande</span></a>
    <p class="panier-garanties"><span>Préparée sous 12 h · Expédiée sous 24 h ouvrées après paiement · Retours sous 14 jours</span><a href="boutique.html">Continuer mes achats</a></p>
  </div>`;
...
const formCode = /** @type {HTMLFormElement} */ (tiroir.querySelector(".panier-code"));
const champCode = /** @type {HTMLInputElement} */ (tiroir.querySelector("#panier-code-champ"));
const messageCode = /** @type {HTMLElement} */ (tiroir.querySelector(".panier-code-message"));
const recapCode = /** @type {HTMLElement} */ (tiroir.querySelector(".panier-code-recap"));
const ligneSousTotal = /** @type {HTMLElement} */ (tiroir.querySelector(".panier-recap-sous-total"));
const retirerCode = /** @type {HTMLButtonElement} */ (tiroir.querySelector(".panier-code-retirer"));
if (codeLienInconnu) { champCode.value = codeLienInconnu; champCode.setAttribute("aria-invalid", "true"); messageCode.textContent = "Code inconnu"; }
...
function rendrePanier() {
  ...
  if (!ids.length) {
    ...
    <div class="panier-cadeau"><label class="panier-cadeau-choix"><input type="checkbox" id="panier-cadeau-actif" ${cadeau.actif ? "checked" : ""}><span><strong>Emballage cadeau</strong> <em>offert</em><small>Boîte, ruban et votre mot, recopié à la main.</small></span></label><div class="panier-cadeau-mot" ${cadeau.actif ? "" : "hidden"}><label for="panier-mot">Votre mot</label><textarea id="panier-mot" rows="3" maxlength="200" placeholder="Quelques lignes, recopiées à la main sur une carte.">${echapper(cadeau.mot)}</textarea><small>Aucun prix dans le colis · <span class="panier-mot-compte">${cadeau.mot.length}</span>/200</small></div></div>
    ...
    <p class="panier-note">«&nbsp;Passer commande&nbsp;» ouvre le formulaire&nbsp;: rien n’est payé sur le site, le comptoir confirme la disponibilité et le total par e-mail.</p>`;
  }
  ...
  const avecCode = Boolean(code) && ids.length > 0;
  formCode.hidden = !ids.length || Boolean(code);
  messageCode.hidden = !ids.length;
  recapCode.hidden = !avecCode;
  ligneSousTotal.classList.toggle("est-secondaire", avecCode);
  if (avecCode) {
    recapCode.querySelector(".panier-code-libelle").textContent = `Code ${code.nom} ${pourcentCode(code)}`;
    recapCode.querySelector(".panier-code-montant").textContent = `− ${euros(remiseCode())}`;
    recapCode.querySelector(".panier-total-montant").textContent = euros(totalAPayer());
  }
...
function fermerPanier() {
  ...
  tiroir.setAttribute("aria-hidden", "true");
  tiroir.inert = true;
  if (messageCode.classList.contains("visuellement-cache")) messageCode.textContent = "";
...
boutonCommande?.addEventListener("click", (event) => {
  ...
  signal("commande", { total: totalAPayer(), articles: nombreArticles(), lignes: Object.entries(panier).map(([id, q]) => ({ id, q })), code: code ? code.nom : "" });
  /* Déjà sur le formulaire : l’ancre seule change, le tiroir doit se refermer pour la laisser voir. */
  if (/\/contact(\.html)?$/.test(window.location.pathname) && params.get("panier") === "1") fermerPanier();
});
/* ---------- Code de réduction saisi dans le tiroir ----------
   « Appliquer » reste un simple bouton : la touche Entrée soumet quand même ce formulaire à champ unique,
   et le seul bouton d’envoi de la page contact reste celui de la commande. */
function appliquerCode() {
  const saisie = champCode.value.trim();
  if (!saisie) { champCode.focus(); return; }
  const trouve = trouverCode(saisie);
  messageCode.classList.toggle("visuellement-cache", Boolean(trouve));
  if (!trouve) {
    champCode.setAttribute("aria-invalid", "true");
    messageCode.textContent = "Code inconnu";
    signal("code", { code: "inconnu", source: "", origine: "saisie" });
    return;
  }
  code = trouve;
  ecrireCode(trouve);
  ...
  messageCode.textContent = `Code ${trouve.nom} appliqué : − ${euros(remiseCode())}.`;
  signal("code", { code: trouve.nom, source: trouve.source, origine: "saisie" });
  rendrePanier();
  retirerCode.focus({ preventScroll: true });
}
formCode.addEventListener("submit", (event) => { event.preventDefault(); appliquerCode(); });
tiroir.querySelector(".panier-code-appliquer")?.addEventListener("click", appliquerCode);
champCode.addEventListener("input", () => {
  if (!champCode.hasAttribute("aria-invalid")) return;
  champCode.removeAttribute("aria-invalid");
  messageCode.textContent = "";
});
retirerCode.addEventListener("click", () => {
  code = null;
  ecrireCode(null);
  messageCode.classList.add("visuellement-cache");
  messageCode.textContent = "Code retiré.";
  rendrePanier();
  champCode.focus({ preventScroll: true });
});
...
if (bandeau && codeDepuisLien) {
  bandeau.innerHTML = `<span class="annonce-long">Code ${codeDepuisLien.nom} appliqué&nbsp;: ${pourcentCode(codeDepuisLien)} sur votre panier, hors livraison.</span><span class="annonce-court">Code ${codeDepuisLien.nom}&nbsp;: ${pourcentCode(codeDepuisLien)} appliqué</span>`;
  bandeau.classList.add("annonce-rappel");
} else if (bandeau && nombreArticles() > 0 && !/\/contact(\.html)?$/.test(location.pathname) && !sessionStorage.getItem("ml-rappel")) {
  const n = nombreArticles();
  bandeau.innerHTML = `<span>Votre panier vous attend : ${n} article${n > 1 ? "s" : ""}, ${euros(totalAPayer())}.</span> <button class="annonce-lien" type="button">Reprendre</button>`;
  bandeau.classList.add("annonce-rappel");
...
window.addEventListener("storage", (event) => { if (event.key === CLE_PANIER || event.key === CLE_CADEAU || event.key === CLE_CODE) { panier = lirePanier(); cadeau = lireCadeau(); code = lireCode(); rendrePanier(); } });
...
function afficherVue(galerie, src, alt, legende) {
  ...
  photo.src = src;
  photo.alt = alt;
  // fondu court : sans lui, la photo change d'un coup et l'œil lit un saut (retour de Mazen le 19/09)
  photo.classList.remove("vue-change");
  void photo.offsetWidth;
  photo.classList.add("vue-change");
...
document.querySelectorAll("[data-courriel], [data-telephone]").forEach((lien) => {
  ...
  if (bloc instanceof HTMLElement) bloc.hidden = !valeur;  // les cartes téléphone naissent masquées dans le HTML : on les révèle si un numéro existe
  if (!valeur) return;
...
if (formulaire instanceof HTMLFormElement) {
  ...
  const libelleAdresse = formulaire.querySelector('label[for="adresse"]');
  if (soumettre instanceof HTMLButtonElement) soumettre.disabled = false;
  if (choix instanceof HTMLSelectElement) {
    /* Menu tiré du catalogue : chaque article y figure, et les homonymes se distinguent par leur format. */
    const noms = Object.values(CATALOGUE).map((p) => p.nom);
    Object.entries(CATALOGUE).forEach(([id, p]) => {
      const homonyme = noms.indexOf(p.nom) !== noms.lastIndexOf(p.nom);
      choix.add(new Option(`${p.nom}${homonyme ? ` (${p.format})` : ""} · ${euros(p.prix)}`, id));
    });
  }
  ...
  const lignesTotaux = () => [
    `Sous-total : ${euros(totalPanier())}`,
    ...(code ? [`Code ${code.nom} (${pourcentCode(code)}) : − ${euros(remiseCode())}`, `Total : ${euros(totalAPayer())}`] : []),
  ];
  const avantagesAcquis = () => [
    ...(totalPanier() >= PALIER_LIVRAISON ? ["livraison offerte"] : ["hors frais de livraison"]),
    ...(totalPanier() >= PALIER_CADEAU ? ["sachet de lavande et roll-on d’huile essentielle offerts"] : []),
  ];
  ...
  if (recap) {
    ...
    if (adresse instanceof HTMLTextAreaElement) adresse.required = true;
    if (libelleAdresse) libelleAdresse.textContent = "Adresse de livraison *";
    const titreFormulaire = document.getElementById("formulaire-titre");
    if (titreFormulaire) titreFormulaire.textContent = "Vos coordonnées";
  }
  ...
  function actualiserDemande() {
    ...
    const avantages = avantagesAcquis().map((a) => `, ${a}`).join("");
    const totaux = code
      ? `<p>Sous-total : <b>${euros(totalPanier())}</b></p><p>Code ${code.nom} (${pourcentCode(code)}) : <b>− ${euros(remiseCode())}</b></p><p>Total : <b>${euros(totalAPayer())}</b>${avantages}.</p>`
      : `<p>Sous-total : <b>${euros(totalPanier())}</b>${avantages}.</p>`;
    const contenuRecap = rempli ? `<strong>Votre panier</strong><ul>${lignesPanier().map((l) => `<li>${l}</li>`).join("")}</ul>${totaux}` : '<strong>Votre panier est vide.</strong><p><a href="boutique.html">Choisir des articles dans la boutique</a> pour préparer votre demande.</p>';
  ...
  if (soumettre instanceof HTMLButtonElement) soumettre.textContent = modePanier ? "Envoyer ma commande" : COORDONNEES.courriel ? "Écrire au comptoir" : "Télécharger ma demande";
  ...
  else if (noteEnvoi instanceof HTMLElement && modePanier) noteEnvoi.textContent = "Le bouton ouvre votre messagerie avec votre commande déjà rédigée : il ne reste qu’à l’envoyer. Le comptoir vous répond sous 48 h ouvrées avec le moyen de paiement.";
  formulaire.addEventListener("submit", (event) => {
    ...
    const livraison = String(donnees.get("adresse") || "").trim();
    /* Commande, adresse et cadeau avant le message libre : la coupure à 1 800 caractères ne touche que le message. */
    const lignes = [
      ...
      ...(piece ? [`Article : ${piece}`] : []),
      ...(modePanier && Object.keys(panier).length ? ["", "Panier :", ...lignesPanier(), ...lignesTotaux(), `${avantagesAcquis().join(", ").replace(/^./, (c) => c.toUpperCase())}.`] : []),
      ...(livraison ? ["", "Adresse de livraison :", livraison] : []),
    ...
    if (COORDONNEES.courriel) {
      ...
      const corps = texte.length > 1800 ? `${texte.slice(0, 1799)}…` : texte;
      window.location.href = `mailto:${COORDONNEES.courriel}?subject=${encodeURIComponent(objet)}&body=${encodeURIComponent(corps)}`;
      if (statut) statut.textContent = `Votre messagerie s’ouvre avec votre ${modePanier ? "commande prête" : "message prêt"} à partir. Rien ne s’ouvre ? Écrivez-nous à ${COORDONNEES.courriel}.`;
      return;
    }
...
document.querySelectorAll(".galerie").forEach((galerie) => {
  ...
  // preventScroll : sinon le navigateur remonte la page jusqu'à la photo à la fermeture de la loupe (200 px mesurés sur iPhone)
  dialogue.addEventListener("close", () => ouvrir.focus({ preventScroll: true }));
});
...
/* ---------- Confirmation par le comptoir : pied de page, panier, fiche produit ----------
   Aucun paiement en ligne (CGV, articles 4 et 5) : la commande part par e-mail et le comptoir répond
   avec le moyen de paiement. Pas de logo de carte ni de « paiement sécurisé » tant que ce n’est pas vrai. */
function blocConfirmation(classe, texte) {
  const bloc = document.createElement("p");
  bloc.className = `confirmation ${classe}`;
  bloc.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="1.5"/><path d="M3.5 6l8.5 7 8.5-7"/></svg><span>${texte}</span>`;
  return bloc;
}
const CONFIRMATION = "Le comptoir confirme votre commande par e-mail sous 48&nbsp;h ouvrées, avec le moyen de paiement.";
document.querySelector(".pied .copyright")?.before(blocConfirmation("confirmation-pied", `${CONFIRMATION} Aucune coordonnée bancaire n’est saisie sur le site.`));
tiroir.querySelector(".panier-commander")?.after(blocConfirmation("confirmation-panier", CONFIRMATION));
document.querySelector(".produit-info .achat-note")?.after(blocConfirmation("confirmation-fiche", CONFIRMATION));

/* ---------- Galerie : flèches, compteur et glisser à gauche / à droite (page et loupe) ---------- */
/* La vignette active se recentre DANS sa bande, jamais avec scrollIntoView : sur iPhone, « block: nearest »
   remontait la page jusqu'à 40 px à chaque photo (mesuré le 19/09 sur une hauteur Safari réelle). */
/* Un appui sur une flèche ou une vignette donne le focus au bouton, et le navigateur remonte alors la page
   pour le dégager de l'en-tête collant (`scroll-padding-top`) : jusqu'à 40 px de saut à chaque photo, mesuré le
   19/09 sur une hauteur d'iPhone. On rend la position au doigt et à la souris ; au clavier, le recentrage reste. */
function garderLaPage(zone) {
  let y = null;
  zone.addEventListener("pointerdown", () => { y = window.scrollY; }, true);
  zone.addEventListener("focusin", (e) => {
    const cible = e.target;
    if (y === null || !(cible instanceof HTMLElement) || cible.matches(":focus-visible")) { y = null; return; }
    if (Math.abs(window.scrollY - y) > 1) window.scrollTo({ top: y, behavior: "instant" });
    y = null;
  });
}
function amenerVignette(vue) {
  const bande = vue.parentElement;
  if (!(bande instanceof HTMLElement) || bande.scrollWidth <= bande.clientWidth + 1) return;
  const gauche = vue.offsetLeft - (bande.clientWidth - vue.offsetWidth) / 2;
  bande.scrollTo({ left: Math.max(0, gauche), behavior: matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" });
}
document.querySelectorAll(".galerie").forEach((galerie) => {
  ...
  const aller = (sens) => {
    ...
    suivant.click();
    amenerVignette(suivant);
    peindre();
    const loupe = document.querySelector("dialog.zoom-produit[open] img");
    const photo = galerie.querySelector("[data-image-principale]");
    if (loupe instanceof HTMLImageElement && photo instanceof HTMLImageElement) {
      loupe.src = photo.src; loupe.alt = photo.alt;
      loupe.classList.remove("vue-change"); void loupe.offsetWidth; loupe.classList.add("vue-change");
    }
  };
